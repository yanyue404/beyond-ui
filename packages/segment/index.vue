  ...
  <div class="result-main">
    ...
    <ul class="result-nav" :class="{'pos-nav':positionFlag}">
      ...
      <li class="in-per-line">
        ...
        <span class="_actline" :style="{'left': line_left, 'width': line_width + 'rem'}"></span>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    //点击tab标题
    clickTab(index = 0) {
      let offTopList = this.getOffetList();
      let t =  Math.min(this.$el.offsetTop + offTopList[index] - this.navHeight + 1, (document.documentElement.scrollHeight || document.body.scrollHeight) - innerHeight);
    ...
    onScroll(){
      ...
      offTopList.forEach((offsetTop, index) => {
        if (Math.ceil(scrollTop) >= offsetTop + this.$el.offsetTop - this.navHeight) {
          activeIndex = index;
        }
      });
      //仅当滑动至tab切换时触发
